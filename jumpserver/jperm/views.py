...
from jperm.utils import gen_keys, gen_keys_by_name
...
def perm_role_add(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            try:
                get_name_id = getpwnam(name)
            except Exception, e:
                raise ServerError(u'LDAP中没有这个用户')
            ...
            if key_content:
                try:
                    key_path = gen_keys_by_name(key=key_content, name=name)
                except SSHException, e:
                    raise ServerError(e)
            else:
                key_path = gen_keys_by_name(name=name)
...
def perm_role_edit(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if key_content:
                try:
                    key_path = gen_keys_by_name(key=key_content, key_path_dir=role.key_path, name=role_name)
                except SSHException:
