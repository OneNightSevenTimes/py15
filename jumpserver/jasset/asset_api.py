...
from jasset.models import ASSET_STATUS, ASSET_TYPE, ASSET_ENV, IDC, AssetRecord, AssetService
...
def db_asset_alert(asset, username, alert_dic):
    ...
    for field, value in alert_dic.iteritems():
        ...
        if field == 'idc':
            ...
            old, new = [], []
            for service_id in value[0]:
                service_name = AssetService.objects.get(id=int(service_id)).name
                old.append(service_name)
            for service_id in value[1]:
                service_name = AssetService.objects.get(id=int(service_id)).name
