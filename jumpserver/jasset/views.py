...
def group_add(request):
    ...
    添加资产组
    """
    header_title, path1, path2 = u'添加部门', u'资产管理', u'添加部门'
...
def group_edit(request):
    ...
    编辑资产组
    """
    header_title, path1, path2 = u'编辑部门', u'资产管理', u'编辑部门'
...
def group_list(request):
    ...
    列出资产组
    """
    header_title, path1, path2 = u'查看部门', u'资产管理', u'查看部门'
...
def asset_list(request):
    ...
    if keyword:
        asset_find = asset_find.filter(
            ...
            Q(parent_ip__contains=keyword) |
            Q(manager=keyword) |
            Q(system_version__contains=keyword))
        # 当资产属于多个部门的时候，会查询到多条记录，需要将结果去重
            ...
            if asset:
                ...
                if group:
                    ...
                    if not set(group_new) < set(group_old):
                        ...
                        alert_list.append([u'部门名称', ','.join(group_old_name), ','.join(group_new_name)])
                if cabinet:
